...
function OneSignal(keys){
  ...
  this.send = function(data, callback) {
   ...
   debug(context.keys.app_id, context.keys.api_key, context.keys.api_key, data.deviceToken, data.message, ios)
   debug("getOneSignalRegisterOptions", getOneSignalRegisterOptions(context.keys.app_id, ios, data.deviceToken))
    // appId, apiKey, token, message
   debug("getOneSignalSendOptions", getOneSignalSendOptions(context.keys.app_id, context.keys.api_key, data.deviceType, data.deviceToken, data.message))
    request( getOneSignalRegisterOptions(context.keys.app_id, ios, data.deviceToken), function(err, result) {
      ...
      request( getOneSignalSendOptions(context.keys.app_id, context.keys.api_key, data.deviceType, data.deviceToken, data.message), function(err, result) {
        if (err) {
...
getOneSignalSendOptions = function(appId, apiKey, deviceType, token, message) {
  var options = {
  ...
  switch (deviceType) {
    // case "android": options.body.isAndroid = true; break;
    // case "ios": options.body.isIos = true; break;
    case "android": options.body["include_android_reg_ids"] = [token]; break;
    case "ios": options.body["include_ios_tokens"] = [token]; break;
    default: return
  }
  return options;
}
