...
function MessagingClient(type) {
  ...
  this.initServices = function(keys) {
    debug("init services", process.env.MANDRILL_API_KEY, keys)
    var context = this
    if(!keys || keys == {}) {
        context.services["mandrill"] = new Mandrill({api_key: process.env.MANDRILL_API_KEY})
        context.services["parse"] = new Parse({app_id: process.env.PARSE_APP_ID, api_key: process.env.PARSE_API_KEY})
        context.services["twilio"] = new Twilio({account_sid: process.env.TWILIO_ACCOUNT_SID, auth_token: process.env.TWILIO_AUTH_TOKEN, from_phone: process.env.TWILIO_FROM_PHONE})
    } else {
