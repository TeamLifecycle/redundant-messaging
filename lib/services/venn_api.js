...
getKeysFromDB = function (apikey, type, callback) {
  debug("getKeysFromDB", apikey, type);
  getUserApp(apikey, function (err, userapp) {
    if (err) callback(err, null);
    debug("getUserApp", userapp);
    context.IntegrationType.findOne({slug: type}, function (err, integrationType) {
      debug("IntegrationType", integrationType);
      if (err) callback(err, null);
      if (!userapp || !userapp.organization) return callback({"message": "no userapp org"})
      ...
      context.OrgIntegration.find({organization: userapp.organization, "integration.type._id": integrationType._id, "active": true}, function (err, appIntegrations) {
        debug("OrgIntegration", userapp._id, integrationType._id, appIntegrations);
        if (err) callback(err, null);
...
postLogFromDB = function (apikey, log, callback) {
  getUserApp(apikey, function (err, userapp) {
    ...
    log.forEach(function (entry) {
      context.OrgIntegration.findOne({'organization': userorg._id, 'integration.slug': entry.service}, function (err, orgIntegration) {
        if (orgIntegration) {
          // context.AppLog.create({'organization': userorg._id, 'orgIntegration': orgIntegration._id, 'code': entry.code, 'message': entry.message}, function (err, appLog) {
